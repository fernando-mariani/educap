...
import { Aulas, Tarefa, Turma, TurmaDetails } from '@/types';
...
export default function DetalhesTurma() {
  ...
  const { getAulas, getTarefas, getTurmas, getTurmasDetails } = useContext(ProfessorContext);
  const [data, setData] = useState<Turma>({} as Turma);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const turmaData = await getTurmas();
      if (turmaData) {
        setData(turmaData.find((t: Turma) => t.id === id) || {} as Turma);
      }

      const aulaData = await getAulas();
      if (aulaData) {
        setAulas(aulaData.filter((a: Aulas) => a.turma.id === id));
      }
      const tarefaData = await getTarefas();
      if (tarefaData) {
        setTarefas(tarefaData.filter((t: Tarefa) => t.turma.id === id));
      }
      const turmaDetailsData = await getTurmasDetails(id);
      if (turmaDetailsData) {
  ...
  if (!data?.id || !turmaDetails?.id) return null;
          ...
          >
            <SafeAreaView edges={['top']} style={styles.headerContent}>
              ...
              <View style={styles.titleWrapper}>
                <Text style={styles.className}>{data.nomeTurma}</Text>
                <View style={styles.subInfoRow}>
                  <View style={styles.tag}>
                    <Text style={styles.tagText}>Educap</Text>
                  </View>
                  <Text style={styles.studentCount}>• {data.numAlunos || 0} Alunos</Text>
                </View>
              </View>
            </SafeAreaView>
          </LinearGradient>
