...
export default function AulasManager() {
  ...
  const { getAulas, getAulasTemplates } = useContext(ProfessorContext);
  const [aulas, setAulas] = useState<Aulas[]>([] as Aulas[]);
  const [template, setTemplate] = useState<AulaTemplate>();
...
const getColorBySubject = (subject: string) => {
    ...
    return { 
      ...
      border: template !== undefined ? template.cor : PRIMARY_COLOR 
    };
  };
...
const ClassBlock = ({ item }: { item: any }) => {
  ...
  const height = (item.horarioFim - item.horarioInicio) * MINUTE_HEIGHT - 4;
