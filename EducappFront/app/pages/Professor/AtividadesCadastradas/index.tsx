...
export default function Atividades() {
    ...
    const { deletarTarefa, getTarefas, getTurmas, getTiposTarefas } = useContext(ProfessorContext);
  ...
  const [selectedTask, setSelectedTask] = useState<typeof tarefas[0] | null>(null);

  useFocusEffect(
    useCallback(() => {
      async function initData() {
        const [dataTarefas, dataTurmas, dataTipos] = await Promise.all([
          getTarefas(),
          getTurmas(),
          getTiposTarefas()
        ]);
        ...
        if (dataTurmas) setTodasTurmas(dataTurmas);
  ...
  // Lógica de Filtragem e Ordenação
    const filteredTarefas = useMemo(() => {
      ...
      if(filterTurma) {
        result = result.filter(t => 
          t.turma.nomeTurma.trim().toUpperCase() === filterTurma.toUpperCase()
        );
      }
    ...
    }, [tarefas, filterTipo, sortAscending, searchText, filterTurma]);
  ...
  const handleOpenDeleteModal = (item: typeof tarefas[0]) => {
        ...
        >
          ...
          onPress={() => {
            if(isExpired) {
              ...
              navigation.navigate('EditarAtividades', { id: item.id })
            }
          }}
        >
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.filterContainer}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScroll}>
              <PressableScale 
                style={[styles.filterChip, !filterTurma && styles.filterChipActive, { marginRight: 8 }]}
                onPress={() => setFilterTurma(null)}
              >
                <Animated.Text entering={FadeInLeft.delay(200)} style={[styles.filterText, !filterTurma && styles.filterTextActive]}>Todas Turmas</Animated.Text>
              </PressableScale>
              {todasTurmas.map((t, i) => (
                <PressableScale 
                  key={t.id}
                  style={[styles.filterChip, filterTurma === t.nomeTurma && styles.filterChipActive, { marginRight: 8 }]}
                  onPress={() => setFilterTurma(t.nomeTurma)}
                >
                  <Animated.Text entering={FadeInLeft.delay(250 + i * 50)} style={[styles.filterText, filterTurma === t.nomeTurma && styles.filterTextActive]}>{t.nomeTurma}</Animated.Text>
                </PressableScale>
              ))}
            </ScrollView>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyStateText}>Tente ajustar seus filtros ou crie uma nova atividade para começar.</Text>
                <TouchableOpacity style={styles.emptyStateButton} onPress={() => { navigation.navigate('CriarAtividade') }}>
                  <Text style={styles.emptyStateButtonText}>Criar Atividade</Text>
                </TouchableOpacity>
              </View>
            }
          />
            ...
            <Pressable style={styles.modalOverlay} onPress={() => setModalVisible(false)}>
              <Pressable style={styles.modalContent} onPress={() => {}}>
                ...
                <Text style={styles.modalMessage}>
                  Tem certeza que deseja excluir a tarefa <Text style={{ fontWeight: 'bold' }}>"{selectedTask?.nomeTarefa}"</Text>?
