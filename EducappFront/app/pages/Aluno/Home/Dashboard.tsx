...
//Cards reutilizaveis
const SummaryCard = React.memo(({ title, value, icon, color, delay = 0, onClick }: any) => (
  <Animated.View 
    entering={FadeInUp.delay(delay).springify()} 
    style={styles.flex1}
  >
    <PressableScale style={styles.smallCard} accessibilityLabel={`${title}: ${value}`} onPress={onClick}>
...
export default function Dashboard() {
  ...
  const { getAulas, getTarefas, aluno } = useContext(AlunoContext);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.greetingBadge}>
          <Text style={styles.greetingText}>Bom dia, {aluno.nome.split(' ')[0]} 👋</Text>
        </View>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.summarySection}>
          <View style={styles.summaryGridLeft}>
            <SummaryCard title="Tarefas" value={logic.pendingCount} icon="edit-3" color={THEME.colors.warning} delay={0} onClick={() => navigation.navigate("Atividades", { screen: "Atividades" })}/>
            <SummaryCard title="Aulas" value={logic.remainingCount} icon="calendar" color={THEME.colors.primary} delay={100} onClick={() => navigation.navigate("Aulas")}/>
          </View>
